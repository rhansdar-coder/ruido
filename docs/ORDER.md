...
| the reconciliation | **built, and fed.** `reconcile()` keeps `forwarded`, `short`, `missing` and `unmatched` apart, and `npm run reconcile:commission` hands it a real chain read: `transfersTo` walks the block range and compares the recipient **locally**, because a filter that fails on the node's side returns an empty page and an empty page reads as "no provider ever forwarded". The expected second legs come from a FILE — nothing in this repository indexes reveals — and the tool refuses an empty list, an unreadable chain or a range that scanned no events rather than printing defaulters |
